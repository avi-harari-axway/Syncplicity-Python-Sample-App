...
class Links:
    ...
    def CreateLink(self, Syncpoint_ID, VirtualPath, SharedLinkPolicy=3, PasswordPolicy=0, ExpirationPolicy=1, ExpirationDays='2'):
        login_headers = {'As-User': '%s' % self.AsUser, 'Accept': 'application/json', "Content-Type": "application/json"}
        url = "syncpoint/links.svc/"
        Method = "POST"
        data = [{"SyncPointId": "%s" % Syncpoint_ID, "VirtualPath": "%s" % VirtualPath, "ShareLinkPolicy": SharedLinkPolicy, "PasswordProtectPolicy": PasswordPolicy, "LinkExpirationPolicy": ExpirationPolicy, "LinkExpireInDays": "%s" % ExpirationDays}]
